...
export default function DashboardPage() {
  ...
  const load = async () => {
    ...
    const [propertyResults, inquiryData, statsData] = await Promise.all([
      Promise.all(
        statuses.map((status) =>
          api.getProperties({
            status,
    ...
    const allProperties = propertyResults.flatMap(
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (editing) {
        await api.updateProperty(editing, payload, token)
      } else {
    ...
    <div className="site-shell">
      ...
      <main className="dashboard-page">
        ...
        <div className="dashboard-stats">
          ...
          <div>
            <strong>{stats.activeProperties || 0}</strong>
